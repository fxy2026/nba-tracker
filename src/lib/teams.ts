...
// Nickname first: NBA nicknames are unique league-wide as whole tokens, which
// the space padding enforces — "Hor-nets" must not match the Nets. City is only
// a fallback, and never the 2-char "LA" — "Or-la-ndo" must not match the Clippers.
export function findTeamByDisplayName(displayName: string): TeamMeta | null {
  if (!displayName) return null;
  const padded = ` ${displayName.toLowerCase()} `;
  for (const meta of Object.values(TEAM_META)) {
    if (padded.includes(` ${meta.name.toLowerCase()} `)) return meta;
  }
  for (const meta of Object.values(TEAM_META)) {
    if (meta.city.length > 3 && padded.includes(` ${meta.city.toLowerCase()} `)) return meta;
  }
  return null;
}
