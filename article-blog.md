<!--
=========================================================================
 NotionNext 博客版 — 复制此文件内容到 Notion 新建页面即可

 推荐 Notion 数据库 property:
   - title:    NBA Tracker v2：一次 UI 大改造，顺手补了一堆坑
   - slug:     nba-tracker-v2-ui-redesign
   - type:     Post
   - status:   Published
   - date:     2026-05-17
   - category: 技术分享
 ...
 操作:
   1. 打开 Notion 博客数据库
   2. 新建一行，填入上面的 property
   3. 进入新建的页面，把下方"=== 正文开始 ==="到"=== 正文结束 ==="
      之间的内容全选复制粘贴
   4. Notion 会自动把 markdown 转成原生 block
   5. 在 Notion 里 publish, NotionNext 会拉取并渲染
=========================================================================
=== 正文开始 ===
-->
...
这篇记录这次 UI 改造的关键节点。代码全部开源在 [github.com/fxy2026/nba-tracker](https://github.com/fxy2026/nba-tracker)。
...
- **field 的名字 ≠ field 的含义**。一个叫 `pts` 的字段可能是生涯均值，可能是上赛季，也可能是当前赛季。名字从来不会告诉你它实际是什么——永远验证 schema。
- **小 bug 是冰山**。手机端「更多」菜单滚不动这种小问题，往往挂着 body scroll lock 缺失、max-height 没设、overscrollBehavior 没配置、两套实现两份翻译漂移**一整套问题**。
- **本地时区是 first-class concept**。函数签名上写出来，不要让"哪个 timezone"成为隐含的、由调用者随机决定的参数。
- **CSS 已经能做你以为只能 JS 做的事**。`:has()`、容器查询、滚动驱动动画、`text-wrap: balance` 都是这两年的新能力。别再写 scroll listener 算进度条了。
