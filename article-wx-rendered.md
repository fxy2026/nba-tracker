...
[上一篇](https://nba.xpy.me/article)写到 NBA Tracker 第一版：8800 行、16 个页面、纯 SVG 投篮图、Suspense 流式渲染。那是季后赛刚开始的时候，我以为已经"做完"了。

然后有一天午后，我躺在床上用手机点开站点，准备看一下今天哪些队打。

底栏五个按钮：今日 / 日历 / 数据 / 搜索 / 更多。点了一下"更多"。

弹层从底部"嗖"地升上来——

【截图：手机端"更多"菜单原版 bug · 已修复前的样子】

弹层顶部撑到了状态栏，下面那点点也看不全。我手指按住屏幕想往下滚——**结果滚动的是下层页面**，弹层本身像粘在那里一动不动。

烦了。

我抬头看了眼时间：周六下午两点。今天本来想躺平的。

这个 bug 看着像 30 分钟能修好，结果一通排查下来发现：

- 弹层没设 `max-height` 和 `overflow-y-auto`
- 弹层打开时 body 没锁 `overflow: hidden`，iOS Safari 触摸事件直接穿透到下层
- 移动端的"更多"菜单还在用一套**全英文**的硬编码翻译，跟桌面端**完全两套实现**

修着修着发现，这个站点的"完美"远没做到位。每个角落都有可以改的地方。

这一篇就是接下来三天我把站点从 8800 行写到 30000 行的记录。

代码全部开源在 `github.com/fxy2026/nba-tracker`，每一处都可以对照看。
...
📷 截图建议：https://nba.xpy.me/

【图片：首页桌面端 — 文件 01-homepage-desktop.png】

依然是当日比赛 + 季后赛对阵图的组合，比分 30 秒自动刷新，不用刷页面。

但**最上面多了一条 2 像素的进度条**，跟着滚动位置慢慢填满，纯 CSS 实现，零 JS：

📷 截图建议：在任意页滚到中间，看顶部 → https://nba.xpy.me/standings

【图片：滚动驱动进度条 + "X 分钟前更新"标签 — 文件 25-scroll-progress-updated-pill.png】

旁边还有一行小字："X 分钟前更新"——告诉你这页数据有多新。每个数据页都标着。

往下滚还有"最近浏览"卡片，把你刚点开过的球员/球队/比赛缩成一行横向滚动：

📷 截图建议：先访问几个 https://nba.xpy.me/player/2544 / https://nba.xpy.me/team/LAL / https://nba.xpy.me/game/0042500411，回首页滚到中部
...
📷 截图建议：滚到首页底部 → https://nba.xpy.me/

【图片：对阵图全貌 — 文件 30-bracket-tree.png】

每个系列卡片可以点进去，跳到独立的系列赛详情页：

📷 截图建议：https://nba.xpy.me/series/004250010 （西部首轮 1）

【图片：系列赛详情页 — 文件 31-series-detail.png】

页面包含逐场战果、双方场均、最大胜差、关键球员排行——把整个 best-of-7 系列赛拍扁到一个页面看。这是新加的页面。

### 历史排行：终于是真的历史了

打开 `/all-time-leaders`，按"生涯场均得分"排序。在第一版里，你会看到——
...
NBA 历史上能场均超过 30 分的只有两个人：乔丹 30.12、张伯伦 30.07。剩下所有人都在 30 以下。Luka 上赛季 33.5 是他**一个赛季**的数据，不是生涯。

为啥会这样？后面技术部分细说。

现在修好了：

📷 截图建议：https://nba.xpy.me/all-time-leaders 然后点"生涯总得分" tab
...
📷 截图建议：https://nba.xpy.me/glossary （记得切到中文模式）
...
`/quiz` 第一版有 3 个模式：看头像猜人 / 看数据猜人 / 猜球队。这次加了第 4 个：**猜历史名人**。

从那 45 位 GOAT 里随机出题，给生涯均值，让你 4 选 1：

📷 截图建议：https://nba.xpy.me/quiz 然后切到第 4 个 tab

【图片：Legend Quiz 模式 — 文件 28-legend-quiz.png】

知道乔丹生涯场均 30.12 是一回事，看到一组 `30.07 / 22.9 / 4.4 / 退役` 能马上认出是张伯伦——是另一回事（22.9 篮板就是大杀器：除了张伯伦只有比尔拉塞尔 22.5）。算是个 NBA 历史小测验。

### PWA：能装到手机主屏

整站可以"安装到主屏"，装好之后就是一个独立 App，没有浏览器 chrome：

📷 截图建议：用 Chrome / Edge / 安卓浏览器开 https://nba.xpy.me，等 InstallPrompt 弹出
...
📷 截图建议：装站点后 DevTools 切 offline 刷新
...
第一版站点的痛点：点进一个数据页（比如 `/streaks` 连胜连败榜）看完只能浏览器后退。

现在每个数据页底部都有"继续探索"区，5-6 个相关链接，覆盖到全站 33 个分析页：

📷 截图建议：滚到 https://nba.xpy.me/standings 底部

【图片：RelatedPages 卡片网格 — 文件 24-related-pages.png】

详情页顶部还多了 breadcrumbs：

📷 截图建议：https://nba.xpy.me/player/2544 顶部（LeBron 的页面）
...
- **中文外号**：字母哥、约老师、大胡子、阿杜、库里、利拉德、浓眉、卡哇伊、东契奇、威少、大帝、塔图姆、文班、蚂蚁、崔阳、胖虎、司机、麦迪、纳什...
- **英文外号**：King James、Greek Freak、Chef Curry、Dame、AD、The Beard、KD、The Beard...
- **传奇球员**：乔丹、科比、张伯伦、魔术师、拉里伯德、奥尼尔、邓肯、艾弗森、奥拉朱旺、邓肯...
- **球队名直接搜**：搜"湖人"或"Lakers"返回整队现役球员

📷 截图建议：https://nba.xpy.me/search 或者按 Cmd+K 用搜索框
...
第一版的对阵图 `BracketTree.tsx` 是个 911 行的怪物。打开就让人头晕：

![BracketTree 拆分前](article-images/mermaid-update-02-bracket-before.png)

每次想动一个细节——比如改卡片的种子数显示位置——都要在这 911 行里翻很久才能找到对应位置。React 组件、纯函数、桌面布局、移动布局全部混在一个文件里。
...
`game/[id]/page.tsx`（比赛详情页）也用同样思路拆：**1026 行 → 243 行 + 13 个 `_components/` 子组件**。`team/[tricode]/page.tsx`：**786 行 → 295 行 + 6 个子组件**。

> Next.js 的 `_components/` 约定：在 App Router 里，下划线开头的文件夹**不会被识别为路由**。所以可以放在路由文件夹下作为同 colocate 的私有组件——只在比赛页用到的子组件就该和它一起住，不污染全局 `src/components/` 命名空间。

拆分的最大收益不是行数减少，是**单元的认知负担降低**——以后维护这个区域不需要把 900 行装进脑子。

### 时区：一个 bug 渗透到全栈

这次重构的真正起点不是手机端 bug，是时区。
...
修复扩散到 `DateNav.tsx`、`GamesList.tsx`、`/api/calendar`、`/app/calendar/page.tsx` 等多处。每一处都用 `Intl.DateTimeFormat` + 用户时区，不用 `new Date().toISOString()`（那个返回 UTC）也不用强转 ET 的 helper。
...
>
> 第一版把这两个概念混在一起，结果就是开头那个 bug。

### 手机端"更多"菜单的修复

回到开头那个手机 bug。表面上是"弹层撑出屏幕"，但 root cause 不止一个：

![弹层 bug 结构](article-images/mermaid-update-06-mobile-bug.png)

但更深的问题是**两套实现两套翻译**：桌面 Navbar 的"更多"菜单已经全中文翻译完了，手机的 MobileNav **重新写了一套**英文硬编码版本。维护两份必然漂移。
...
桌面 Navbar 和手机 MobileNav 都 `const moreGroups = useMoreGroups();`，传给同一个 `<CommandPalette>` 组件。手机端从底部抽屉切换到**居中模态**：
...
Chrome 122+ 加了新 API，能告诉浏览器：哪些 URL 值得预先 prefetch 或者 prerender。
...
**Bug 让用户能感知到问题（页面挂了，数据没出来）。标签错让用户带着错误信息走**。后者更难发现，影响更大。
...
*这次更新基于 https://mdnice.com 排版，所有架构图（mermaid）已本地渲染。*
